...
def MatrixCreator(w, t, q0, phi):
    ...
    M = np.array(
        [
            [
                np.cos(w * t) ** 2,
                1j * qstar * np.cos(w * t) * np.sin(w * t),
                -1j * q * np.cos(w * t) * np.sin(w * t),
                0.5 * (-1 + q0**2) + q0**2 * np.sin(w * t) ** 2,
            ],
            [
                1j * np.cos(w * t) * np.sin(w * t),
                qstar * np.cos(w * t) ** 2,
                q * np.sin(w * t) ** 2,
                -1j * q0**2 * np.cos(w * t) * np.sin(w * t),
            ],
            [
                -1j * np.cos(w * t) * np.sin(w * t),
                qstar * np.sin(2 * t) ** 2,
...
def FixedwAndt(w, t, q0_range, phi_range):
    storage = np.zeros(
        (len(q0_range), len(phi_range), 3)
    )  # 3 because I store q0, phi and the truth value
