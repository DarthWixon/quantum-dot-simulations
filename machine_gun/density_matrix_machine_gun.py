...
def PerfectMachineGunDensityMatrices(n_photons):
    ...
    U_y = qutip.qobj.Qobj(
        mgb.SingleQubitOperationCreator(rotation_operator, n_photons + 1, 1)
    )
...
def DephasingMachineGun(n_photons, dephasing_value):
    ...
    for photon_index in range(n_photons):
        CNOT_superop = qutip.superop_reps.to_super(CNOT_array[photon_index])
        density_matrix_vec = (
            U_y_superop * CNOT_superop * dephasing_superop * density_matrix_vec
        )
...
def AmplitudeDampingMachineGun(n_photons, damping_value):
    ...
    for photon_index in range(n_photons):
        CNOT_superop = qutip.superop_reps.to_super(CNOT_array[photon_index])
        density_matrix_vec = (
            U_y_superop * CNOT_superop * damping_superop * density_matrix_vec
        )
...
def SimulationTimeTest(max_photons, n_trials):
    ...
    pyplot.errorbar(
        photon_count_array, mean_array, yerr=std_dev_array, linestyle="None", marker="."
    )
...
def BasicDephasingFidelityTest(
    n_photons, n_dephasing_values=10, graphing=False, min_dephase=0, max_dephase=1
):
    perfect_result = PerfectMachineGunDensityMatrices(n_photons)
    fidelity_array = numpy.zeros(n_dephasing_values)

    dephasing_values_array = numpy.linspace(
        min_dephase, max_dephase, n_dephasing_values
    )
...
def BasicDephasingTraceDistTest(
    n_photons, n_dephasing_values=10, graphing=False, min_dephase=0, max_dephase=1
):
    perfect_result = PerfectMachineGunDensityMatrices(n_photons)
    ...
    if graphing:
        ...
        pyplot.title(
            "Trace Distance vs Perfect State (for {} photon MG)".format(n_photons)
        )
...
def BasicDampingFidelityTest(
    n_photons, n_damping_values=10, graphing=False, min_damping=0, max_damping=1
):
    perfect_result = PerfectMachineGunDensityMatrices(n_photons)
    ...
    for i in range(n_damping_values):
        imperfect_result = AmplitudeDampingMachineGun(
            n_photons, damping_values_array[i]
        )
        fidelity_array[i] = qutip.metrics.fidelity(perfect_result, imperfect_result)
...
def ManyDephasingFidelityTests(
    max_photons, n_dephasing_values=20, min_dephase=0, max_dephase=1, output=False
):
    ...
    for j in range(max_photons):
        big_fidelity_array[j] = BasicDephasingFidelityTest(
            j,
            n_dephasing_values=n_dephasing_values,
            min_dephase=min_dephase,
            max_dephase=max_dephase,
        )
        if not output:
            pyplot.plot(
                dephasing_values_array,
                big_fidelity_array[j],
                label="{} Qubit State".format(j + 1),
                color=colour_list[j],
            )
...
def ManyDephasingTraceDistTests(
    max_photons, n_dephasing_values=10, min_dephase=0, max_dephase=1, output=False
):
    ...
    for j in range(max_photons):
        big_tracedist_array[j] = BasicDephasingFidelityTest(
            j,
            n_dephasing_values=n_dephasing_values,
            min_dephase=min_dephase,
            max_dephase=max_dephase,
...
def FidelityAndTraceDistanceDephasing(
    max_photons, n_dephasing_values=10, min_dephase=0, max_dephase=1
):
    big_tracedist_array = ManyDephasingTraceDistTests(
        max_photons, n_dephasing_values, min_dephase, max_dephase, output=True
    )
    big_fidelity_array = ManyDephasingFidelityTests(
        max_photons, n_dephasing_values, min_dephase, max_dephase, output=True
    )
    dephasing_values_array = numpy.linspace(
        min_dephase, max_dephase, n_dephasing_values
...
def ManyDampingFidelityTests(
    max_photons, n_damping_values=20, min_damping=0, max_damping=1
):
    big_fidelity_array = numpy.zeros((max_photons, n_damping_values))
    damping_values_array = numpy.linspace(min_damping, max_damping, n_damping_values)

    for j in range(max_photons):
        big_fidelity_array[j] = BasicDampingFidelityTest(
            j,
            n_damping_values=n_damping_values,
            min_damping=min_damping,
            max_damping=max_damping,
        )
        pyplot.plot(
            damping_values_array,
            big_fidelity_array[j],
            label="{} Photon State".format(j),
        )
...
def MaximumDephasingFidelityTest(max_photons):
    ...
    for i in range(max_photons):
        ...
        results_array[i] = (
            i + 1,
            qutip.metrics.fidelity(perfect_result, imperfect_result),
            1 / numpy.sqrt(i + 1),
            1 / (2 ** (i / 2)),
            1 / numpy.sqrt(2 ** (i + 1)),
        )
...
def Ruth2DDephasingFidelity(
    max_photons, n_dephasing_values, min_dephase=0, max_dephase=1
):
    results_array = numpy.zeros((n_dephasing_values, max_photons))

    for p in range(max_photons):
        results_array[:, p] = BasicDephasingFidelityTest(
            p, n_dephasing_values, min_dephase=min_dephase, max_dephase=max_dephase
        )  # p+1 because we countr from 0
