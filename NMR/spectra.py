...
def calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds = [100,1200, 439, 880], data_only_return = False, rf_field = 5e-3, use_sundfors_GET_vals = False):
	...
	eta_array, V_XX_array, V_YY_array, V_ZZ_array, euler_angles_array = bqf.load_calculated_EFG_arrays(nuclear_species, region_bounds, use_sundfors_GET_vals = use_sundfors_GET_vals)
	...
	quadrupole_coupling_constant = (3*e*Q)/(2*h*spin*(2*spin - 1)) # constant to convert Vzz to frequency units
	...
	for r, rf_freq in enumerate(rf_freq_list):
		perms = permutations(index_list, 2) # because of how iterators work in python, we must recreate this each time
		for pair in perms:
			...
			transition_rate_data[r] += transition_rate_calc(rf_Ham, init_state, final_state, init_energy, final_energy, rf_freq)
...
def varied_B_field_spectra(nuclear_species, applied_field_list, field_geometry, rf_freq_list, location, region_bounds = [100,1200, 439, 880]):
	spectra_data_list = []

	for applied_field in applied_field_list:
		spectra_data_list.append(calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds))

	plt.figure()

	for spectra_data_dict in spectra_data_list:
		applied_field = spectra_data_dict["applied_field"]
		x_points = spectra_data_dict["rf_freq_list"]/1e6 # divide by 1e6 so that it plots in MHz
	...
	ax = plt.gca()
	plt.title(f"{nuclear_species}, Various Applied Fields, {field_geometry} Orientation")
...
def varied_isotope_spectra(applied_field, field_geometry, rf_freq_list, location, region_bounds = [100, 1200, 439, 880]):
	spectra_data_list = []
...
def many_field_colourplot(nuclear_species, applied_field_list, field_geometry, rf_freq_list, location, region_bounds = [100, 1200, 439, 880], saving = False, graphing = True):
	data = np.zeros((len(applied_field_list), len(rf_freq_list)))

	for b, applied_field in enumerate(applied_field_list):
		data[b,:] = calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds, data_only_return = True)

	data = np.log(data)

	if graphing:
		img_extent = [rf_freq_list[0]/1e6, rf_freq_list[-1]/1e6, applied_field_list[0], applied_field_list[-1]]

		plt.imshow(data, origin = "lower", cmap = cm.GnBu, aspect = "auto", extent = img_extent, interpolation = "none")
...
def many_location_colourplot(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_bounds = [100, 1200, 439, 880], saving = False):
	data = np.zeros((len(applied_field_list), len(rf_freq_list)))
...
def parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, use_sundfors_GET_vals = False, rf_field = 5e-3):
	...
	zipped_params = list(zip(nuclear_species_list, applied_field_list_for_zip, field_geometry_list, rf_freq_list_list, loc_list, region_bounds_list, data_only_return_list, rf_field_list, use_sundfors_GET_vals_list))

	return zipped_params

def many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc = False, use_sundfors_GET_vals = False):
	...
	for b, applied_field in enumerate(applied_field_list):
		with multiprocessing.Pool() as pool:
			zipped_params = parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, use_sundfors_GET_vals)
			slices_across_locations = pool.starmap(calculate_absorbtion_single_slice_data, zipped_params)
			pool.close()
...
def load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals = False):
...
def many_location_plot_using_loading(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label = "entire_dot", saving = False, recalc = False, use_sundfors_GET_vals = False):
	
	# print(f"function is many_location_plot_using_loading")
	# print(f"nuclear_species = {nuclear_species}")
	# print(f"field_geometry = {field_geometry}")

	many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc, use_sundfors_GET_vals)

	data = load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals)
...
def many_NMR_spectra_data_calculation(nuclear_species_list, applied_field_list, rf_freq_list, loc_list, region_label, recalc = False):
	# start = time.time()

	for nuclear_species in nuclear_species_list:
		for field_geometry in ["Voigt", "Faraday"]:
			many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc)
...
def many_location_parallel_plot(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label = "entire_dot", region_bounds = [100, 1200, 439, 880], saving = False, graphing = True):
	region_bounds = bqf.QD_regions_dict[region_label]
	...
	for b, applied_field in enumerate(applied_field_list):
		with multiprocessing.Pool() as pool:
			zipped_params = parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds)
			slices_across_locations = pool.starmap(calculate_absorbtion_single_slice_data, zipped_params)
			pool.close()
...
def random_locations_list_generator(n_locs, region_label):
	...
	eta_array, V_XX_array, V_YY_array, V_ZZ_array, euler_angles_array = bqf.load_calculated_EFG_arrays("Ga69", region_bounds) # use Ga69 cos we only care about the size of the arrays
...
def Faraday_Voigt_spectra_comparison(nuclear_species, applied_field_list, rf_freq_list, loc_list, region_label = "entire_dot", saving = False, recalc = False):
	many_location_parallel_calculation(nuclear_species, applied_field_list, "Voigt", rf_freq_list, loc_list, region_label, recalc)
	many_location_parallel_calculation(nuclear_species, applied_field_list, "Faraday", rf_freq_list, loc_list, region_label, recalc)

	voigt_data = load_many_location_data(nuclear_species, applied_field_list, "Voigt", rf_freq_list, loc_list, region_label)		
	faraday_data = load_many_location_data(nuclear_species, applied_field_list, "Faraday", rf_freq_list, loc_list, region_label)		
	...
	plt.imshow(data, origin = "lower", cmap = cm.seismic, aspect = "auto", extent = img_extent, interpolation = "none", vmin = -scale, vmax = scale)
...
def side_by_side_comparison_plotter(nuclear_species, applied_field_list, rf_freq_list, loc_list, region_label = "entire_dot", saving = False, recalc = False, use_sundfors_GET_vals = False):
	
	for field_geometry in ["Faraday", "Voigt"]:
		many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc, use_sundfors_GET_vals)

	faraday_data = np.log(load_many_location_data(nuclear_species, applied_field_list, "Faraday", rf_freq_list, loc_list, region_label, use_sundfors_GET_vals))
	voigt_data = np.log(load_many_location_data(nuclear_species, applied_field_list, "Voigt", rf_freq_list, loc_list, region_label, use_sundfors_GET_vals))

	fig, (ax1, ax2) = plt.subplots(nrows = 1, ncols = 2, figsize = (20, 10))

	img_extent = [rf_freq_list[0]/1e6, rf_freq_list[-1]/1e6, applied_field_list[0], applied_field_list[-1]]

	ax1.imshow(faraday_data, origin = "lower", cmap = cm.GnBu, aspect = "auto", extent = img_extent, interpolation = "none")
	ax2.imshow(voigt_data, origin = "lower", cmap = cm.GnBu, aspect = "auto", extent = img_extent, interpolation = "none")
...
def side_by_side_GET_comparison_plotter(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label = "entire_dot", saving = False, recalc = False):
	
	for use_sundfors_GET_vals in [True, False]:
		many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc, use_sundfors_GET_vals)

	chek_data = np.log(load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals = False))
	sundfors_data = np.log(load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals = True))

	fig, (ax1, ax2) = plt.subplots(nrows = 1, ncols = 2, figsize = (20, 10))

	img_extent = [rf_freq_list[0]/1e6, rf_freq_list[-1]/1e6, applied_field_list[0], applied_field_list[-1]]

	ax1.imshow(chek_data, origin = "lower", cmap = cm.GnBu, aspect = "auto", extent = img_extent, interpolation = "none")
	ax2.imshow(sundfors_data, origin = "lower", cmap = cm.GnBu, aspect = "auto", extent = img_extent, interpolation = "none")
...
def Sundfors_Checkovich_comparison(nuclear_species, applied_field_list, rf_freq_list, loc_list, region_label, field_geometry, saving = False, recalc = False):
	many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc, use_sundfors_GET_vals = True)
	many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, recalc, use_sundfors_GET_vals = False)

	old_GET_data = load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals = True)		
	new_GET_data = load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label, use_sundfors_GET_vals = False)
	...
	cax = ax.imshow(data, origin = "lower", cmap = cm.RdBu, aspect = "auto", extent = img_extent, interpolation = "none") #, vmin = -scale, vmax = scale)
	
	cbar = fig.colorbar(cax, orientation = "vertical", ticks = [0.9*data_min, 0, 0.9*data_max], shrink = 0.95)
	cbar.ax.set_yticklabels(["Old GET \nDominates", "Similar \nValues", "New GET \nDominates"])
...
def testing_parallel_speedup(nuclear_species, applied_field_list, field_geometry, rf_freq_list, location, loc_list, region_bounds = [100, 1200, 439, 880]):
	# timing if parallel is faster
	# tests done on a 100 * 100 grid, without showing or saving any graphs

	start = time.time()
	many_field_colourplot(nuclear_species, applied_field_list, field_geometry, rf_freq_list, location, region_bounds, saving = False, graphing = False)
	end = time.time()
	taken = np.around((end-start)/60, 2)
	print(f"Single calc using non parallel function took {taken} minutes.")

	start = time.time()
	many_location_parallel_plot(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_bounds, saving = False, graphing = False)
	end = time.time()
	taken = np.around((end-start)/60, 2)
...
def large_hd_spectra(nuclear_species, loc_list, region_label, field_geometry, recalc, saving = True):
	min_freq = 0
	max_freq = 100 # in MHz
	n_freqs = 1000
	...
	many_location_plot_using_loading(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label = region_label, saving = saving, recalc = recalc)

def region_characterisation_spectra(nuclear_species, region_label, field_geometry, recalc, saving = True):
	min_freq = 0
	max_freq = 75 # in MHz
	n_freqs = 600
	min_field = 0.001 # to stop divide by 0 errors when taking logs (16/12/20)
	...
	search_range = 25 # search an area of size 0.6nm**2 around each site to find an atom
	...
	many_location_plot_using_loading(nuclear_species, applied_field_list, field_geometry, rf_freq_list, best_loc_list, region_label = region_label, saving = saving, recalc = recalc)
...
def comparison_spectra(nuclear_species, loc_list, region_label, recalc, saving = True):
	...
	Faraday_Voigt_spectra_comparison(nuclear_species, applied_field_list, rf_freq_list, loc_list, region_label = region_label, saving = saving, recalc = recalc)

def GET_values_comparison_spectra(nuclear_species, field_geometry, loc_list, region_label, recalc, saving = True):
	min_freq = 0
	max_freq = 75 # in MHz
	n_freqs = 600
	min_field = 0.001 # to stop divide by 0 errors when taking logs (16/12/20)
	...
	Sundfors_Checkovich_comparison(nuclear_species, applied_field_list, rf_freq_list, loc_list, region_label, field_geometry, saving = saving, recalc = recalc)
	side_by_side_GET_comparison_plotter(nuclear_species, applied_field_list, field_geometry, rf_freq_list, loc_list, region_label = "entire_dot", saving = saving, recalc = recalc)
...
def experimental_NMR_sim(field_geometry, region_label, n_locs, recalc, saving = True):
	...
	n_In_locs = int(np.ceil(mean_In_conc * n_locs)) # find how many In atoms there are, floor to keep as an integer
	...
	for i, nuclear_species in enumerate(["In115", "Ga69", "As75"]):
		...
		many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, species_loc_list, region_label, recalc)
		data += load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, species_loc_list, region_label)
...
def experimental_NMR_sim_transparent_version(field_geometry, region_label, n_locs, recalc, saving = False):
	sys.path.append("/home/will/Documents/work/research/simulations/concentration/")
	import conc_maps as conc
	...
	alpha_list = [1, 0.5, 0.25] # decreasing so that we don't see only red or blue lines

	for i, nuclear_species in enumerate(["In115", "Ga69", "As75"]):
		species_loc_list = location_lists[i]
		many_location_parallel_calculation(nuclear_species, applied_field_list, field_geometry, rf_freq_list, species_loc_list, region_label, recalc)
		data.append(np.log(load_many_location_data(nuclear_species, applied_field_list, field_geometry, rf_freq_list, species_loc_list, region_label)))
		ax.imshow(data[i], origin = "lower", cmap = cmap_list[i], aspect = "auto", extent = img_extent, interpolation = "none", alpha = alpha_list[i])
...
def gigahertz_range_spectra(nuclear_species, loc_list, region_label, field_geometry, recalc, saving = True):
...
def single_field_line_spectra(nuclear_species, applied_field, loc_list, region_label, field_geometry, recalc, rf_field_strength = 5e-3, saving = False, returning = False):
	...
	for location in loc_list:
		absorbtion_data += calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds, data_only_return = True, rf_field = rf_field_strength)

	absorbtion_data = absorbtion_data/np.amax(absorbtion_data)

	fig, ax = plt.subplots(figsize = (16,10))
	ax.plot(rf_freq_list/1e6, absorbtion_data, color = bqf.will_dark_blue)
	ax.set_title(f"{nuclear_species}, {np.around(applied_field, 2)}T Applied Field, {field_geometry} Orientation")
	ax.set_xlabel("RF Frequency (MHz)")
	ax.set_ylabel("Absorbtion (arb. units)")
	ax.text(0.75, 0.85, f"{len(loc_list)} Nuclei \n{rf_field_strength}T RF Field", transform = ax.transAxes)
...
def applied_field_comparison_line_spectra(nuclear_species, applied_field_list, loc_list, region_label, field_geometry, recalc, rf_field_strength = 5e-3, saving = False, in_parallel = False):
	...
	if os.path.isfile(filename):
		...
		if in_parallel:
			for a, applied_field in enumerate(applied_field_list):
				with multiprocessing.Pool() as pool:
					zipped_params = parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, use_sundfors_GET_vals = False, rf_field = rf_field_strength)
					slices_across_locations = pool.starmap(calculate_absorbtion_single_slice_data, zipped_params)
					pool.close()
			...
			for a, applied_field in enumerate(applied_field_list):
				for location in loc_list:
					absorbtion_data[a,:] += calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds, data_only_return = True, rf_field = rf_field_strength)
	...
	for a, ax in enumerate(axs):
		...
		# ax.set_ylabel("Absorbtion (arb. units)")
		# ax.set_xlabel("RF Frequency (MHz)")
		ax.text(0.75, 0.5, f"{applied_field_list[a]}T Applied Field", transform = ax.transAxes)
...
def RF_comparison_line_spectra(nuclear_species, applied_field, loc_list, region_label, field_geometry, rf_field_list, recalc = False, saving = True, in_parallel = False):
	...
	if os.path.isfile(filename):
		...
		if in_parallel:
			for r, rf_field in enumerate(rf_field_list):
				with multiprocessing.Pool() as pool:
					zipped_params = parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, use_sundfors_GET_vals = False, rf_field = rf_field)
					slices_across_locations = pool.starmap(calculate_absorbtion_single_slice_data, zipped_params)
					pool.close()
			...
			for r, rf_field in enumerate(rf_field_list):
				for location in loc_list:
					absorbtion_data[r,:] += calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds, data_only_return = True, rf_field = rf_field)
	...
	for a in range(n_RF_fields):
		ax.plot(rf_freq_list/1e6, absorbtion_data[a,:], label = f"{rf_field_list[a]:.2e}")
...
def number_of_nuclei_comparison(nuclear_species, applied_field, n_nuclei_list, region_label, field_geometry, rf_field, recalc = False, saving = True, in_parallel = False):
		...
		if in_parallel:
			for n, n_nuclei in enumerate(n_nuclei_list):
				...
				with multiprocessing.Pool() as pool:
					...
					slices_across_locations = pool.starmap(calculate_absorbtion_single_slice_data, zipped_params)
					pool.close()
				absorbtion_data[n,:] = np.sum(slices_across_locations, axis = 0)/n_nuclei

		else:
			for n, n_nuclei in enumerate(n_nuclei_list):
				loc_list = random_locations_list_generator(n_nuclei, region_label)
				for location in loc_list:
					absorbtion_data[n,:] += calculate_absorbtion_single_slice_data(nuclear_species, applied_field, field_geometry, rf_freq_list, location, region_bounds, data_only_return = True, rf_field = rf_field)
...
def load_combined_absorbtion_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei):
...
def overlaid_pulse_graphs(pulse_location, pulse_width, field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei, saving = False):
	absorbtion_data = load_combined_absorbtion_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei)
	rf_freq_list = np.linspace(min_freq, max_freq, n_freqs) * 1e6

	pulse = create_lorentzian_pulse(rf_freq_list, pulse_location*1e6, pulse_width*1e6)
	pulse = pulse/np.amax(pulse)
	multiplied_absorption = absorbtion_data * pulse

	fig, ax = plt.subplots(figsize = (16, 10))

	ax.plot(rf_freq_list/1e6, pulse, label = "Pulse", linestyle = "dotted", color = bqf.will_light_blue)
	ax.plot(rf_freq_list/1e6, multiplied_absorption, label = "Absorbtion", color = bqf.will_dark_blue)
	ax.text(0.05, 0.95, rf" {pulse_location} $\pm$ {pulse_width/2}", transform = ax.transAxes)
	ax.legend()
...
def grid_overlaid_pulse_graphs(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei, saving = False):
	absorbtion_data = load_combined_absorbtion_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei)
	rf_freq_list = np.linspace(min_freq, max_freq, n_freqs) * 1e6
	...
	fig, axs = plt.subplots(len(pulse_location_range), len(pulse_width_range), figsize = (16,10))
	...
	for m, pulse_location in enumerate(pulse_location_range):
		for s, pulse_width in enumerate(pulse_width_range):
			...
			ax.plot(rf_freq_list/1e6, multiplied_absorption, label = "Absorbtion", color = bqf.will_dark_blue)
			ax.axis("off")
			ax.text(0.05, 0.95, rf" {pulse_location} $\pm$ {pulse_width/2}", transform = ax.transAxes, fontsize = 24)
...
def combined_species_single_spectra_data(field_geometry, min_freq = 0, max_freq = 100, n_freqs = 1000, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 1200):
	region_bounds = bqf.QD_regions_dict[region_label]
	loc_list = random_locations_list_generator(n_nuclei, region_label)
...
def combined_species_single_spectra_plot(field_geometry, min_freq = 0, max_freq = 100, n_freqs = 1000, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 1200, saving = False):
	
	data = load_combined_absorbtion_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei)
...
def combined_species_single_spectra_plot_subregion(field_geometry, target_min = 70, target_max = 90, min_freq = 0, max_freq = 100, n_freqs = 1000, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 1200, saving = False):
	...
	fig, ax = plt.subplots(figsize = (16, 10))
	ax.plot(rf_freq_list[target_min_arg : target_max_arg]/1e6, data[target_min_arg : target_max_arg], color = bqf.will_dark_blue)
	...
	if target_min == 70 and target_max == 90:
		...
		ax.axhspan(0, 1.1*np.amax(data[target_min_arg : target_max_arg]), xmin = shaded_min, xmax = shaded_max, color = "red", alpha = 0.5)
...
def pulse_graph_area_calculations(field_geometry, pulse_location, pulse_width, min_freq = 0, max_freq = 100, n_freqs = 1000, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 1200):
	rf_freq_list = np.linspace(min_freq, max_freq, n_freqs) * 1e6
	absorbtion_data = load_combined_absorbtion_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei)
	...
	area_under_pulse = scipy.integrate.simpson(multiplied_absorption, rf_freq_list/1e6)
...
def comparison_area_plots(field_geometry, pulse_location, pulse_width_range, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 1200, saving = False):
	...
	rf_freq_list = np.linspace(overall_min_freq, overall_max_freq, overall_n_freqs) * 1e6
	...
	large_side_min_index, _ = array_find_nearest(rf_freq_list/1e6, large_side_min_freq)
	large_side_max_index, _ = array_find_nearest(rf_freq_list/1e6, large_side_max_freq)

	absorbtion_data = load_combined_absorbtion_data(field_geometry, overall_min_freq, overall_max_freq, overall_n_freqs, applied_field, overall_n_nuclei)

	spectrum_area = scipy.integrate.simpson(absorbtion_data, rf_freq_list/1e6)
	pre_pulse_target_area = scipy.integrate.simpson(absorbtion_data[pulse_min_index : pulse_max_index], rf_freq_list[pulse_min_index : pulse_max_index]/1e6)
	pre_pulse_side_area = scipy.integrate.simpson(absorbtion_data[side_min_index : side_max_index], rf_freq_list[side_min_index : side_max_index]/1e6)
	pre_pulse_large_side_area = scipy.integrate.simpson(absorbtion_data[large_side_min_index : large_side_max_index], rf_freq_list[large_side_min_index : large_side_max_index]/1e6)
	...
	# as of 10/12/21 I need to make the plots of target area/initial target area and of the rest of it - as discussed with Andrew
		...
		area_in_target_region = scipy.integrate.simpson(final_spectrum[pulse_min_index : pulse_max_index], rf_freq_list[pulse_min_index : pulse_max_index]/1e6)
		area_in_side_region = scipy.integrate.simpson(final_spectrum[side_min_index : side_max_index], rf_freq_list[side_min_index : side_max_index]/1e6)
		area_in_large_side_region = scipy.integrate.simpson(final_spectrum[large_side_min_index : large_side_max_index], rf_freq_list[large_side_min_index : large_side_max_index]/1e6)
	...
	plt.plot(pulse_width_range, target_area_data/pre_pulse_target_area, color = bqf.will_dark_blue, label = "77.5 - 82.5")
	# plt.plot(pulse_width_range, target_saturation_data, color = bqf.will_light_blue, label = "Transition Saturation")
	plt.plot(pulse_width_range, side_area_data/pre_pulse_side_area, color = bqf.will_light_blue, label = "50-70 MHz")
	plt.plot(pulse_width_range, large_side_area_data/pre_pulse_large_side_area, color = "seagreen", label = "20-40 MHz")
...
if HD_spectra:
	...
	for nuclear_species in bqf.nuclear_species_list:
		for field_geometry in bqf.field_geometries:
			large_hd_spectra(nuclear_species, loc_list, "entire_dot", field_geometry, recalculating)
...
if small_region_spectra:
	for region_label in ["entire_dot"]:
		for nuclear_species in bqf.nuclear_species_list:
			for field_geometry in bqf.field_geometries:
				region_characterisation_spectra(nuclear_species, region_label, field_geometry, True)
...
if creating_comparison:
	for region_label in ["entire_dot"]:
		...
		for nuclear_species in bqf.nuclear_species_list:
			for field_geometry in bqf.field_geometries:
				comparison_spectra(nuclear_species, loc_list, region_label, recalculating)

if experimental_plot:
	n_locs = 24
	for region_label in ["central_high_In", "central_low_In", "below_outside_dot", "entire_dot"]:
		for field_geometry in bqf.field_geometries:
			experimental_NMR_sim(field_geometry, region_label, n_locs, recalculating, True)
	...
	for region_label in ["entire_dot"]:
		...
		for nuclear_species in ["Ga69", "As75"]:
			for field_geometry in ["Faraday"]:
				GET_values_comparison_spectra(nuclear_species, field_geometry, loc_list, region_label, recalculating)
...
if side_by_side_comparison_plots:
	...
	side_by_side_comparison_plotter("Ga69", applied_field_list, rf_freq_list, loc_list, region_label = "entire_dot", saving = True, recalc = False, use_sundfors_GET_vals = False)
	side_by_side_comparison_plotter("Ga71", applied_field_list, rf_freq_list, loc_list, region_label = "entire_dot", saving = True, recalc = False, use_sundfors_GET_vals = False)
	side_by_side_comparison_plotter("As75", applied_field_list, rf_freq_list, loc_list, region_label = "entire_dot", saving = True, recalc = False, use_sundfors_GET_vals = False)
	
	n_locs = 8
	loc_list = random_locations_list_generator(n_locs, "entire_dot")
	side_by_side_GET_comparison_plotter("Ga69", applied_field_list, "Voigt", rf_freq_list, loc_list, region_label = "entire_dot", saving = True, recalc = False)
	side_by_side_GET_comparison_plotter("As75", applied_field_list, "Voigt", rf_freq_list, loc_list, region_label = "entire_dot", saving = True, recalc = False)

if layered_NMR_experiment:
	region_label = "entire_dot"
	for field_geometry in bqf.field_geometries:
		experimental_NMR_sim_transparent_version(field_geometry, region_label, n_locs = 24, recalc = False, saving = True)
...
if gigahertz_spectra:
	...
	for nuclear_species in bqf.nuclear_species_list:
		for field_geometry in bqf.field_geometries:
				gigahertz_range_spectra(nuclear_species, loc_list, region_label, field_geometry, recalc = True, saving = True)
...
if line_spectra:
	...
	for applied_field in applied_field_list:
		for nuclear_species in ["In115"]: # bqf.nuclear_species_list:
			for field_geometry in bqf.field_geometries:
				single_field_line_spectra(nuclear_species, applied_field, loc_list, region_label, field_geometry, recalc = False, rf_field_strength = 5e-3, saving = True)
...
if changing_B_field_line_spectra:
	...
	for nuclear_species in ["In115"]: # bqf.nuclear_species_list:
		for field_geometry in ["Voigt"]: # bqf.field_geometries:
			applied_field_comparison_line_spectra(nuclear_species, applied_field_list, loc_list, region_label, field_geometry, recalc = False, rf_field_strength = 5e-3, saving = True, in_parallel = True)
...
if changing_RF_field_line_spectra:
	...
	for nuclear_species in ["In115"]: # bqf.nuclear_species_list:
		for field_geometry in ["Voigt"]: # bqf.field_geometries:
			RF_comparison_line_spectra(nuclear_species, applied_field, loc_list, region_label, field_geometry, rf_field_list, recalc = False, saving = True, in_parallel = True)
...
if changing_number_of_nuclei:
	...
	for nuclear_species in ["In115"]: #bqf.nuclear_species_list:
		for field_geometry in ["Voigt"]: #bqf.field_geometries:
			number_of_nuclei_comparison(nuclear_species, applied_field, n_nuclei_list, region_label, field_geometry, rf_field = 5e-3, recalc = False, saving = True, in_parallel = True)
...
for n_nuclei in [10000]:
	if all_species_slice_calculation:
		...
		combined_species_single_spectra_data(field_geometry, min_freq, max_freq, n_freqs, applied_field, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = n_nuclei)
	...
	if plotting_all_species_slice:
		...
		combined_species_single_spectra_plot(field_geometry, min_freq, max_freq, n_freqs, applied_field, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = n_nuclei, saving = True)
	...
	if plotting_NMR_slice_subrange:
		...
		for i in range(len(low_freq_list)):
			low_freq = low_freq_list[i]
			high_freq = high_freq_list[i]
			combined_species_single_spectra_plot_subregion(field_geometry, low_freq, high_freq, min_freq = 0, max_freq = 150, n_freqs = 10000, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = 10000, saving = True)
	...
	if applying_pulse:
		...
		for pulse_location in pulse_location_range:
			for pulse_width in pulse_width_range:
				overlaid_pulse_graphs(pulse_location, pulse_width, field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei, saving = True)
				if pulse_area_calcs:
					pulse_graph_area_calculations(field_geometry, pulse_location, pulse_width, min_freq, max_freq, n_freqs, applied_field, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = n_nuclei)
	...
	if grid_apply_pulses:
		...
		# applied_field = 3
		# n_nuclei = 1
		grid_overlaid_pulse_graphs(field_geometry, min_freq, max_freq, n_freqs, applied_field, n_nuclei, saving = True)

	if comparative_area_calcs:
		pulse_location = 80.2
		pulse_width_range = np.linspace(0.01, 20, 1000)

		comparison_area_plots(field_geometry, pulse_location, pulse_width_range, applied_field = 3, region_label = "entire_dot", rf_field_strength = 5e-3, n_nuclei = n_nuclei, saving = True)
