...
def ManyParticlePositionToLengthConverter(
    coords_array, box_width, box_height, n_rows, n_cols
):
    ...
    length_list = np.zeros(
        n_springs
    )  # derivation of this number of springs uses grid graphs and is in my 2nd lab book
    length_list = (
        length_list + 1000
    )  # check to see if I've missed any, they should have a ridiculous value if so
    ...
    for row in range(n_rows):
        for col in range(n_cols):
            ...
            length_list[above_spring_index] = np.sqrt(
                (x_pos - top_x_start) ** 2 + (y_pos - top_y_start) ** 2
            )
            length_list[left_spring_index] = np.sqrt(
                (x_pos - left_x_start) ** 2 + (y_pos - left_y_start) ** 2
            )
            ...
            if row == n_rows - 1:  # check if we're on the last row of atoms
                ...
                length_list[below_spring_index] = np.sqrt(
                    (x_pos - bottom_x_start) ** 2 + (y_pos - bottom_y_start) ** 2
                )
            ...
            if col == n_cols - 1:  # check if we're on the last column of atoms
                ...
                length_list[right_spring_index] = np.sqrt(
                    (x_pos - right_x_start) ** 2 + (y_pos - right_y_start) ** 2
                )
...
def ManyParticlePositionEnergyFunction(
    coords_array,
    ...
    potential = 0
    length_list = ManyParticlePositionToLengthConverter(
        coords_array, box_width, box_height, n_rows, n_cols
    )

    for i in range(len(length_list)):
        potential += (
            spring_constant_list[i] * (length_list[i] - natural_length_list[i]) ** 2
        )
...
def SpringConstantFromAtomicSpecies(large_species_array, real_atoms=True):
    ...
    spring_constant_list = np.zeros(
        2 * small_n_rows * small_n_cols + small_n_rows + small_n_cols
    )
    ...
    for row in range(1, n_rows - 1):
        for col in range(1, n_cols - 1):
            ...
            spring_constant_list[above_spring_index] = bond_strength_dict[
                above_bond_index
            ]
            spring_constant_list[below_spring_index] = bond_strength_dict[
                below_bond_index
            ]
            spring_constant_list[left_spring_index] = bond_strength_dict[
                left_bond_index
            ]
            spring_constant_list[right_spring_index] = bond_strength_dict[
                right_bond_index
            ]
...
def NaturalLengthFromAtomicSpecies(large_species_array, real_atoms=True):
    ...
    for row in range(1, n_rows - 1):
        for col in range(1, n_cols - 1):
            ...
            natural_length_list[above_spring_index] = natural_length_dict[
                above_bond_index
            ]
            natural_length_list[below_spring_index] = natural_length_dict[
                below_bond_index
            ]
            natural_length_list[left_spring_index] = natural_length_dict[
                left_bond_index
            ]
            natural_length_list[right_spring_index] = natural_length_dict[
                right_bond_index
            ]
...
def StrainTensorCalculator(strained_coords_array, unstrained_coords_array):
    ...
    strain_tensor_array = np.zeros(
        (n_rows, n_cols, 2, 2)
    )  # 2x2 because the strain tensor is a 2x2 matrix
        ...
        for col in range(n_cols):
            ...
            for target_row in range(n_rows):
                for target_col in range(n_cols):
                    target_x_unstrained = unstrained_coords_array[
                        target_row, target_col, 0
                    ]
                    target_y_unstrained = unstrained_coords_array[
                    ...
                    unstrained_difference = np.array(
                        [
                            base_x_unstrained - target_x_unstrained,
                            base_y_unstrained - target_y_unstrained,
                        ]
                    )
                    strained_difference = np.array(
                        [
                            base_x_strained - target_x_strained,
                            base_y_strained - target_y_strained,
                        ]
                    )

                    W_matrix += np.tensordot(
                        strained_difference, unstrained_difference, axes=0
                    )
                    V_matrix += np.tensordot(
                        unstrained_difference, unstrained_difference, axes=0
                    )
...
def RowAndColLengthFinder(length_list, n_rows, n_cols):
    ...
    for row in range(n_rows):
        ...
        end_index = (
            start_index + 2 * n_cols + 1
        )  # plus 1 to make sure it includes the last bond
        row_lengths[row] = np.sum(length_list[start_index:end_index:2])

    for col in range(n_cols):
        start_index = 2 * col * n_rows
        end_index = (
            start_index + 2 * n_rows + 1
        )  # plus 1 to make sure it includes the last bond
...
def GaAsLatticeWithIndiumCentre(n_rows, n_cols):
    ...
    for row in range(n_rows + 2):
        for col in range(n_cols + 2):
            if (
                row == int(math.floor(n_rows / 2)) + 1
                and col == int(math.floor(n_cols / 2)) + 1
                and large_species_array[row, col] == 0
            ):
...
def LatticeGenerator(n_rows, n_cols, lattice_type=0, In_positions=[[]]):
    if lattice_type == 0:
        large_species_array, simulated_species_array = GaAsEqualLatticeGenerator(
            n_rows, n_cols
        )
    elif lattice_type == 1:
        large_species_array, simulated_species_array = GaAsLatticeWithIndiumCentre(
            n_rows, n_cols
        )
    elif lattice_type == 2:
        large_species_array, simulated_species_array = GaAsLatticeWithScatteredIndium(
...
def Plotter(
    simulated_species_array,
    unstrained_positions,
    strained_positions,
    time_taken,
    average_row_difference,
    average_col_difference,
    showing_plots=True,
    real_atoms=False,
):
    ...
    ConnectionsPlotter(
        unstrained_positions, "black", "dashed", 0.3, simulated_species_array
    )
    plt.scatter(
    ...
    ConnectionsPlotter(
        strained_positions, "black", "dashed", 0.8, simulated_species_array
    )
    plt.scatter(
        strained_positions[:, :, 0],
        strained_positions[:, :, 1],
        c=colour_array,
        alpha=1,
    )
    ...
    if not real_atoms:
        ...
        legend_elements = [
            ...
            Line2D(
                [0], [0], color="w", markerfacecolor="r", marker="o", label="Ga Atom"
            ),
            Line2D(
                [0], [0], color="w", markerfacecolor="b", marker="o", label="As Atom"
            ),
            Line2D(
                [0], [0], color="w", markerfacecolor="g", marker="o", label="In Atom"
            ),
        ]
    ...
    strain_tensor_array = StrainTensorCalculator(
        strained_positions, unstrained_positions
    )
...
def Simulation(n_rows, n_cols, lattice_type, In_positions=[[]], real_atoms=True):
    ...
    large_species_array, simulated_species_array = LatticeGenerator(
        n_rows, n_cols, lattice_type, In_positions
    )

    spring_constant_list = SpringConstantFromAtomicSpecies(
        large_species_array, real_atoms
    )
    natural_length_list = NaturalLengthFromAtomicSpecies(
        large_species_array, real_atoms
    )
    ...
    start = time.time()
    result = opt.minimize(
        ManyParticlePositionEnergyFunction,
        unstrained_positions,
        args=(
            spring_constant_list,
            natural_length_list,
    ...
    length_list = ManyParticlePositionToLengthConverter(
        strained_positions, box_width, box_height, n_rows, n_cols
    )
    row_lengths, col_lengths = RowAndColLengthFinder(length_list, n_rows, n_cols)
    average_row_difference = np.around(
        (np.sqrt(np.sum(np.square(row_lengths - box_width))) / n_rows)
        * 100
        / box_width,
        decimal_places,
    )
    average_col_difference = np.around(
        (np.sqrt(np.sum(np.square(col_lengths - box_height))) / n_cols)
        * 100
        / box_height,
        decimal_places,
    )
...
if __name__ == "__main__":
    ...
    (
        ...
        average_row_difference,
        average_col_difference,
    ) = Simulation(n_rows, n_cols, lattice_type, In_positions, real_atoms)
    Plotter(
        simulated_species_array,
        unstrained_positions,
        strained_positions,
