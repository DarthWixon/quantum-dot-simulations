...
def plot_strain_image(region_bounds, faked_strain_type, saving = False):
    ...
    xx_array, xz_array, zz_array = bqf.load_mirrored_data(region_bounds, faked_strain_type = faked_strain_type)
...
def get_mirrored_EFG_data(nuclear_species, region_bounds, faked_strain_type):
    ...
    bqf.calculate_and_save_EFG(nuclear_species, region_bounds, use_sundfors_GET_vals = False, real_strain_data = False, faked_strain_type = faked_strain_type)
    eta, V_XX, V_YY, V_ZZ, euler_angles = bqf.load_calculated_EFG_arrays(nuclear_species, region_bounds, use_sundfors_GET_vals = False, real_strain_data = False, faked_strain_type = faked_strain_type)

    return eta, V_XX, V_YY, V_ZZ, euler_angles

def plot_arrows_with_strength_background(nuclear_species, region_bounds, faked_strain_type, saving = False):
    eta, V_XX, V_YY, V_ZZ, euler_angles = get_mirrored_EFG_data(nuclear_species, region_bounds, faked_strain_type)
...
def quadrupolar_strength_distributions(region_bounds, faked_strain_type, saving = False, positive_only = False):
    ...
    for nuclear_species in bqf.nuclear_species_list:
        ...
        eta, V_XX, V_YY, V_ZZ, euler_angles = eta, V_XX, V_YY, V_ZZ, euler_angles = get_mirrored_EFG_data(nuclear_species, region_bounds, faked_strain_type)
        ...
        if positive_only:
            if nuclear_species == "Ga69":
                ...
                quadrupole_frequency_list = quadrupole_frequency_list[quadrupole_frequency_list <= 0]
            else:
                quadrupole_frequency_list = quadrupole_frequency_list[quadrupole_frequency_list >= 0]
        ...
        quadrupole_frequency_arrays.append(quadrupole_frequency_list) # divide by 1e6 to get in MHz

    
    hist_vals, bin_edges, patches = plt.hist(quadrupole_frequency_arrays, "auto", density = True, stacked = False, histtype = "step", label = bqf.nuclear_species_list)
...
def single_histogram_boltzmann_fit(nuclear_species, region_bounds, faked_strain_type, saving = False, positive_only = False):
    species = ISOP.species_dict[nuclear_species]
    spin = species["particle_spin"]
    Q = species["quadrupole_moment"]
    ...
    quadrupole_frequency_list = list(quadrupole_frequency_list) # has to be a list type for a reason I don't remember right now (24/08/21)

    fig, ax = plt.subplots()
    hist_vals, bin_edges, patches = ax.hist(np.abs(quadrupole_frequency_list), "auto", density = True, histtype = "step", label = species["short_name"], color = bqf.will_dark_blue)
    ...
    x = bin_centres
    ax.plot(x, maxwell.pdf(x, *fit_params), linestyle = "dashed", color = bqf.will_light_blue)
...
def all_species_boltzmann_fit(region_bounds, faked_strain_type, saving = False, positive_only = False):
    h = constants.h
    e = constants.e
...
def single_histogram_gamma_fit(nuclear_species, region_bounds, faked_strain_type, saving = False, positive_only = False):
    species = ISOP.species_dict[nuclear_species]
    spin = species["particle_spin"]
    Q = species["quadrupole_moment"]
...
def all_species_gamma_fit(region_bounds, faked_strain_type, saving = False, positive_only = False):
    h = constants.h
    e = constants.e
...
def random_sites_energy_level_diagram(nuclear_species, region_bounds, field_geometry, faked_strain_type, n_sites, min_field = 0, max_field = 2, n_fields = 100, saving = False):
    applied_field_list = np.linspace(min_field, max_field, n_fields)

    eta_array, V_XX_array, V_YY_array, V_ZZ_array, euler_angles_array = get_mirrored_EFG_data(nuclear_species, region_bounds, faked_strain_type)
    ...
    if saving:
        plot_title = "random_sites_energy_level_diagram_of_{}_with_changing_b_field_in_{}_orientation_over_region_{}_in_{}_mirrored_dot.png".format(nuclear_species, field_geometry, region_bounds, faked_strain_type)
        plt.savefig(f"{graph_path}{plot_title}")
    else:
        plt.show()
    plt.close()

def random_sites_geometry_comparison_EL_diagram(nuclear_species, region_bounds, faked_strain_type, n_sites, min_field = 0, max_field = 2, n_fields = 100, saving = False):
    applied_field_list = np.linspace(min_field, max_field, n_fields)
...
def NMR_calculate_absorbtion_single_slice(nuclear_species, applied_field, field_geometry, region_bounds, faked_strain_type, rf_freq_list, location, data_only_return = True, rf_field = 5e-3):
    ...
    for r, rf_freq in enumerate(rf_freq_list):
        ...
        for pair in perms:
            ...
            transition_rate_data[r] += bqf.transition_rate_calc(rf_Ham, init_state, final_state, init_energy, final_energy, rf_freq)
...
def random_locations_list_generator(n_locs, region_bounds):
    ...
    eta_array, V_XX_array, V_YY_array, V_ZZ_array, euler_angles_array = get_mirrored_EFG_data("As75", region_bounds, "left_right") # dummy values just to size arrays lazily
...
def NMR_parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, faked_strain_type):
    ...
    zipped_params = list(zip(nuclear_species_list, applied_field_list_list, field_geometry_list, region_bounds_list, faked_strain_type_list, rf_freq_list_list, loc_list, data_only_return_list, rf_field_list))

    return zipped_params


def NMR_parrallel_calculation(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, loc_list, recalc = False):
    ...
    for b, applied_field in enumerate(applied_field_list):
        with multiprocessing.Pool() as pool:
            zipped_params = NMR_parameter_zipper(nuclear_species, applied_field, field_geometry, rf_freq_list, loc_list, region_bounds, faked_strain_type)
            slices_across_locations = pool.starmap(NMR_calculate_absorbtion_single_slice, zipped_params)
            pool.close()
...
def NMR_load_data(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, loc_list):
    filename = f"{data_path}NMR_absorbtion_data_for_{nuclear_species}_in_{field_geometry}_{len(loc_list)}_locs_in_{region_bounds}_for_{len(applied_field_list)}_B_fields_and_{len(rf_freq_list)}_rf_fields_over_{applied_field_list[0]}_{applied_field_list[-1]}T_{rf_freq_list[0]/1e6}_{rf_freq_list[-1]/1e6}MHz_in_{faked_strain_type}_mirrored_dot.npy"
    spectra_data = np.load(filename)

    return spectra_data

def NMR_plot(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, loc_list, saving = False, recalc = False):
    
    NMR_parrallel_calculation(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, loc_list, recalc = recalc)
    
    data = NMR_load_data(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, loc_list)
...
def NMR_experiment_plot(field_geometry, region_bounds, faked_strain_type, n_locs, recalc = False, saving = False):
    ...
    for i, nuclear_species in enumerate(["In115", "Ga69", "As75"]):
        ...
        NMR_parrallel_calculation(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, species_loc_list, recalc = recalc)

        data.append(np.log(NMR_load_data(nuclear_species, region_bounds, faked_strain_type, applied_field_list, field_geometry, rf_freq_list, species_loc_list)))
...
for r, faked_strain_type in enumerate(["left_right", "right_left"]):
    ...
    all_species_gamma_fit(region_bounds, faked_strain_type, saving = True, positive_only = False)
    all_species_gamma_fit(region_bounds, faked_strain_type, saving = True, positive_only = True)
    ...
    for nuclear_species in bqf.nuclear_species_list:
        ...
        single_histogram_gamma_fit(nuclear_species, region_bounds, faked_strain_type, saving = True, positive_only = False)
        single_histogram_gamma_fit(nuclear_species, region_bounds, faked_strain_type, saving = True, positive_only = True)
