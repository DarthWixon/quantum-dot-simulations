...
def CalculateHamiltonians(nuclear_species, location, applied_field, region_bounds):
	...
	x_coord, y_coord = location # (0,0) is the first element of the region_bounds array, regardless of it's size
	...
	farady_hamiltonian = bqf.structural_hamiltonian_creator_Faraday(Zeeman_term, quadrupolar_term, eta, spin, alpha, beta, gamma)/1e6
	voigt_hamiltonian = bqf.structural_hamiltonian_creator_Voigt(Zeeman_term, quadrupolar_term, eta, spin, alpha, beta, gamma)/1e6

	return farady_hamiltonian, voigt_hamiltonian

def PlotHintonDiagrams(nuclear_species, location, applied_field = 0.5, region_bounds = [100,1200,200,1000], saving = False):
	farady_hamiltonian, voigt_hamiltonian = CalculateHamiltonians(nuclear_species, location, applied_field, region_bounds)
...
def CalculateEigenstructure(nuclear_species, location, applied_field = 0.5, region_bounds = [100,1200,200,1000], printing = False):
	farady_hamiltonian, voigt_hamiltonian = CalculateHamiltonians(nuclear_species, location, applied_field, region_bounds)
	...
	if printing:
		...
		for i in range(len(faraday_eigenvectors)):
			...
			print(f"Overlap method gives: {faraday_eigenvectors[i].overlap(faraday_eigenvectors[i])}")
...
def ParameterPrinter(nuclear_species, location, applied_field = 0.5, region_bounds = [100,1200,200,1000]):
...
def FindingEigenvectorDifferences(faraday_eigenvectors, voigt_eigenvectors):
	...
	for i in range(len(norm_array)):
		norm_array[i] = np.real_if_close(faraday_eigenvectors[i].overlap(voigt_eigenvectors[i]))
...
def EigenvalueDataforDara(nuclear_species, location, applied_field = 1, region_bounds = [100,1200,200,1000]):

	farady_eigenstructure, voigt_eigenstructure = CalculateEigenstructure(nuclear_species, location, applied_field, printing = False)
	...
	if diff >= 0:
		...
		print(f"Faraday Eigenvalues: {np.around(np.real_if_close(faraday_eigenvalues), 1)}")
...
def EigenvectorDifferencesWithChangingField(field_range, location, saving = False):
	...
	for n, nuclear_species in enumerate(["Ga69", "Ga71", "As75", "In115"]):
		for i, applied_field in enumerate(field_range):
			faraday_eigenstructure, voigt_eigenstructure = CalculateEigenstructure(nuclear_species, location, applied_field)
			faraday_eigenvectors, voigt_eigenvectors = faraday_eigenstructure[1], voigt_eigenstructure[1]
			results_array[n, i] = np.abs(FindingEigenvectorDifferences(faraday_eigenvectors, voigt_eigenvectors))
...
def EigenvectorDifferenceWithChangingLocation(applied_field, x_range, y_range, saving = False):
	...
	for n, nuclear_species in enumerate(["Ga69", "Ga71", "As75", "In115"]):
		for x, x_loc in enumerate(x_range):
			for y, y_loc in enumerate(y_range):
				...
				faraday_eigenstructure, voigt_eigenstructure = CalculateEigenstructure(nuclear_species, location, applied_field)
				faraday_eigenvectors, voigt_eigenvectors = faraday_eigenstructure[1], voigt_eigenstructure[1]
				results_array[n, x, y] = np.abs(FindingEigenvectorDifferences(faraday_eigenvectors, voigt_eigenvectors))
