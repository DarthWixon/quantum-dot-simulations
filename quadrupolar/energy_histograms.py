...
def single_histogram_and_fit_guassian(nuclear_species, region_bounds, plotting = False, saving = False, use_sundfors_GET_vals = False):
    ...
    quadrupole_frequency_list = list(K * V_ZZ.flatten()/1e6) # divide by 1e6 to get in MHz

    hist_vals, bin_edges, patches = plt.hist(quadrupole_frequency_list, "auto", density = True, histtype = "step", label = species["short_name"])
    ...
    if plotting:
        ...
        plt.title(f"Nuclear Species: {nuclear_species}, Region: {region_bounds}, Fit Parameters: $\mu$ = {mean}, $\sigma$ = {std_dev}")
...
def single_histogram_and_fit_boltzmann(nuclear_species, region_bounds, plotting = False, saving = False, use_sundfors_GET_vals = False):
    species = ISOP.species_dict[nuclear_species]
    spin = species["particle_spin"]
    Q = species["quadrupole_moment"]
...
def single_histogram_gamma_fit(nuclear_species, region_bounds, use_sundfors_GET_vals, saving = False):
    species = ISOP.species_dict[nuclear_species]
    spin = species["particle_spin"]
    Q = species["quadrupole_moment"]
...
def generate_hist_data(nuclear_species, region_bounds, use_sundfors_GET_vals):
    ...
    np.savez(archive_name, measured_prob_density = hist_vals, frequencies = bin_centres, fit_prob_density = hist_fit, mean = mean, std_dev = std_dev, r2 = r2)
...
def find_all_fits(region_bounds, use_sundfors_GET_vals = False):
    ...
    for nuclear_species in bqf.nuclear_species_list:
        mean, std_dev, r2 = single_histogram_and_fit_guassian(nuclear_species, region_bounds, use_sundfors_GET_vals = use_sundfors_GET_vals)
        print(f"{nuclear_species} has: mean = {mean} MHz & std_dev = {std_dev} MHz, with R2 = {r2}")

    print("---------------------------------")

def quadrupolar_strength_distributions(region_bounds = [100, 1200, 200, 1000], saving = False, use_sundfors_GET_vals = False):
    """
    ...
    for nuclear_species in bqf.nuclear_species_list:
        ...
        K = (3*e*Q)/(2*h*spin*(2*spin - 1)) # constant to convert Vzz to fq
        quadrupole_frequency_arrays.append(K * V_ZZ.flatten()/1e6) # divide by 1e6 to get in MHz
...
def layered_QI_distributions(region_bounds_list, saving = False, use_sundfors_GET_vals = False):
    ...
    for r, region_bounds in enumerate(region_bounds_list):
        ...
        hist_vals, bin_edges, patches = plt.hist(quadrupole_frequency_arrays, "auto", density = True, histtype = "step", label = bqf.nuclear_species_list, color = colour_list, ls = linestyle_list[r], linewidth = 2)

    # use these fake lines to make a legend that is nice
    custom_lines = [Line2D([0], [0], color = colour_list[0], label = bqf.nuclear_species_list[0]),
...
def new_vs_old_comparison_hists(region_bounds_list, saving = False):
    ...
    for r, region_bounds in enumerate(region_bounds_list):
        ...
        for nuclear_species in changed_species_list:
            ...
            if species_count <=1:
                eta, V_XX, V_YY, V_ZZ, euler_angles = bqf.load_calculated_EFG_arrays(nuclear_species, region_bounds, use_sundfors_GET_vals = False)
            else:
                eta, V_XX, V_YY, V_ZZ, euler_angles = bqf.load_calculated_EFG_arrays(nuclear_species, region_bounds, use_sundfors_GET_vals = True)
        ...
        hist_vals, bin_edges, patches = plt.hist(quadrupole_frequency_arrays, "auto", density = True, histtype = "step", label = label_list, color = colour_list, ls = linestyle_list[r], linewidth = 2)

    custom_lines = [Line2D([0], [0], color = colour_list[0], label = label_list[0]),
