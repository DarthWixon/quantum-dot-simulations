...
def single_field_energy_levels(
    applied_field,
    location,
    nuclear_species,
    field_geometry,
    ...
    if field_geometry == "Faraday":
        ...
        print(
            f"Orientation parameter must be either Faraday or Voigt, not {field_geometry}."
        )
        return np.nan
...
def many_fields_energy_levels_calc(
    applied_field_list, location, nuclear_species, field_geometry
):
    ...
    for a, applied_field in enumerate(applied_field_list):
        energy_level_data[:, a] = single_field_energy_levels(
            applied_field, location, nuclear_species, field_geometry
        )

    return energy_level_data


def single_eta_energy_levels(
    eta,
    nuclear_species,
    field_geometry,
    applied_field,
...
def many_eta_energy_levels_calc(
    eta_list, nuclear_species, field_geometry="Faraday", applied_field=0
):
    n_levels = int((2 * ISOP.species_dict[nuclear_species]["particle_spin"]) + 1)
    ...
    for n, eta in enumerate(eta_list):
        energy_level_data[:, n] = single_eta_energy_levels(
            eta, nuclear_species, field_geometry, applied_field
        )
...
def plot_energy_level_diagram(
    ...
    plt.title(
        f"Energy Level Splitting of {nuclear_species} in a {field_geometry} geometry."
    )

    if saving:
        print("Saving graph of changing: {}".format(x_axis_label))
        plot_title = (
            "energy_level_diagram__of_{}_with_changing_{}_in_{}_orientation".format(
                nuclear_species, x_axis_label.replace(" ", "_"), field_geometry
            )
        )
...
def plot_and_save_all_species_EL_diagrams(
    location, field_geometry, region_bounds=[100, 1200, 200, 1000]
):
    applied_field_list = np.linspace(0, 3, 1000)
    eta_list = np.linspace(0, 1, 500)

    for nuclear_species in ["Ga69", "Ga71", "As75", "In115"]:
        full_b_field_data = many_fields_energy_levels_calc(
            applied_field_list, location, nuclear_species, field_geometry
        )
        ...
        full_eta_data = many_eta_energy_levels_calc(
            eta_list, nuclear_species, field_geometry
        )
        plot_energy_level_diagram(
            eta_list,
            full_eta_data,
...
def plot_single_species_both_geometries_EL_diagrams(
    nuclear_species, location, region_bounds=[100, 1200, 200, 1000], saving=False
):
    applied_field_list = np.linspace(0, 3, 1000)
    ...
    full_faraday_data = many_fields_energy_levels_calc(
        applied_field_list, location, nuclear_species, field_geometry="Faraday"
    )
    full_voigt_data = many_fields_energy_levels_calc(
        applied_field_list, location, nuclear_species, field_geometry="Voigt"
    )
...
def all_sites_energy_level_diagram(
    nuclear_species,
    field_geometry,
    min_field=0,
    max_field=2,
    ...
    for x in range(x_dim):
        for y in range(y_dim):
            for i in range(n_levels):
                plt.plot(
                    applied_field_list, data[x, y, i, :], color="black", linewidth=0.5
                )
                # print(energy_level_data[i,:])
    ...
    if saving:
        plot_title = "all_sites_energy_level_diagram__of_{}_with_changing_b_field_in_{}_orientation_over_region_{}".format(
            nuclear_species, field_geometry, region_bounds
        )
...
def random_sites_energy_level_diagram(
    ...
    if saving:
        plot_title = "random_sites_energy_level_diagram__of_{}_with_changing_b_field_in_{}_orientation_over_region_{}".format(
            nuclear_species, field_geometry, region_bounds
        )
        plt.savefig(f"{graph_path}{plot_title}")
        plt.close()
...
random_sites_geometry_comparison_EL_diagram(
    nuclear_species,
    n_sites,
    min_field=0,
    max_field=2,
...
for nuclear_species in bqf.nuclear_species_list:
    plot_single_species_both_geometries_EL_diagrams(
        nuclear_species, location, region_bounds=[100, 1200, 200, 1000], saving=True
    )
    random_sites_geometry_comparison_EL_diagram(
        nuclear_species,
        n_sites,
