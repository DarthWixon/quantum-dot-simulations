...
def equivalent_B_field_calculation(
    nuclear_species, region_bounds=[100, 1200, 200, 1000]
):
    """
...
def single_equivalent_B_field_grapher(
    nuclear_species,
    equivalent_B_field,
    saving=False,
    ...
    species = ISOP.species_dict[
        nuclear_species
    ]  # one of ["Ga69", "Ga71", "As75", "In115"] (as strings)
    fig, ax = plt.subplots()
    im = ax.imshow(equivalent_B_field)
    plt.axis("off")
    plt.colorbar(im)
    plot_name = (
        f"{graph_path}equivalent_B_field_for_"
        + species["short_name"]
        + "_in_region_{}.png".format(str(region_bounds))
    )
...
def all_equivalent_B_field_grapher(
    B_field_arrays, saving=True, region_bounds=[100, 1200, 200, 1000]
):
    """
    ...
    # fig.tight_layout()
    plot_name = (
        f"{graph_path}equivalent_B_field_for_all_elements_in_region_{region_bounds}.png"
    )
    if saving:
        print("Saving plot: {}".format(plot_name))
        plt.savefig(plot_name)
...
def one_time_one_field_all_sites_correlator(
    t,
    applied_field,
    correlator_axis,
    nuclear_species,
    ...
    for s in range(n_sites):
        ...
        euler_angles = data_dict["euler_angles"].reshape(n_sites, 3)[
            s
        ]  # has already been reshaped when I load it (as of 24/08/20 this may no longer be true)
        V_ZZ = data_dict["V_ZZ"].flatten()[s]

        total_correlator += bqf.site_correlator_calculator_not_parallel(
            t,
...
step_size_list = list(
    range(0, 100, 5)
)  # make a list cos python3 returns a range object which doesn't support assignment
    ...
    for nuclear_species in nuclear_speciesList:
        for step_size in step_size_list:
            bqf.calculate_and_save_EFG(
                nuclear_species, region_bounds, step_size=step_size
            )

if creating_equivalent_B_field_graphs:
    print("Creating Graphs")
    for nuclear_species in nuclear_speciesList:
        equivalent_B_field = equivalent_B_field_calculation(
            nuclear_species, region_bounds
        )
        single_equivalent_B_field_grapher(
            nuclear_species,
            equivalent_B_field,
            saving=True,
...
if creating_4_way_B_field_graphs:
    ...
    for i, nuclear_species in enumerate(nuclear_speciesList):
        B_field_arrays.append(
            equivalent_B_field_calculation(nuclear_species, region_bounds)
        )
