...
def parameter_zipper(
    ...
    ) = bqf.load_calculated_EFG_arrays(nuclear_species, region_bounds, step_size)
    ...
    zipped_params = list(
        zip(
            t_list,
            zeeman_frequency_per_tesla_list,
            quadrupole_coupling_constant_list,
            spin_list,
            biaxiality_list,
            alpha_list,
            beta_list,
...
def one_species_time_series_correlator_calculator(
    timerange,
    applied_field,
    nuclear_species,
    region_bounds,
    step_size=100,
    allocated_at_once=25,
):
    """
    ...
    for c, correlator_axis in enumerate(["x", "y", "z"]):
        for t, elapsed_time in enumerate(timerange):
            n_sites, zipped_params = parameter_zipper(
                elapsed_time,
                applied_field,
                correlator_axis,
                nuclear_species,
            ...
            with multiprocessing.Pool() as pool:
                site_correlators = pool.starmap(
                    bqf.parallel_site_correlator_calculator,
                    zipped_params,
                    chunksize=allocated_at_once,
                )
                pool.close()
...
def log_spaced_correlator_simulation(
    min_time_exp,
    max_time_exp,
    n_times,
    applied_field,
    ...
    Ga69_data = one_species_time_series_correlator_calculator(
        timerange, applied_field, "Ga69", region_bounds, step_size, allocated_at_once
    )
    print("Ga69 Log Data Calculated")
    Ga71_data = one_species_time_series_correlator_calculator(
        timerange, applied_field, "Ga71", region_bounds, step_size, allocated_at_once
    )
    print("Ga71 Log Data Calculated")
    As75_data = one_species_time_series_correlator_calculator(
        timerange, applied_field, "As75", region_bounds, step_size, allocated_at_once
    )
    print("As75 Log Data Calculated")
    In115_data = one_species_time_series_correlator_calculator(
        timerange, applied_field, "In115", region_bounds, step_size, allocated_at_once
    )
    ...
    np.savez(
        ...
        Ga71_data=Ga71_data,
        As75_data=As75_data,
        In115_data=In115_data,
    )


def linear_spaced_correlator_simulation(
    min_time,
    max_time,
    timestep,
    applied_field,
...
def single_species_linear_time_correlator_grapher(
    min_time,
    max_time,
    timestep,
    applied_field,
    ...
    plt.title(
        f"Correlator for {nuclear_species} with Applied Field of {applied_field}T"
    )
    plt.legend()
...
def single_species_fourier_transform_grapher(
    min_time,
    max_time,
    timestep,
    applied_field,
    ...
    plt.title(
        f"Fourier Transforms for {nuclear_species} with Applied Field of {applied_field}"
    )
    plt.legend()
...
def single_species_correlator_to_fourier(
    min_time,
    max_time,
    timestep,
    applied_field,
    ...
    correlator_data = one_species_time_series_correlator_calculator(
        timerange,
        applied_field,
        nuclear_species,
        region_bounds,
    ...
    single_species_linear_time_correlator_grapher(
        min_time,
        max_time,
        timestep,
        applied_field,
    ...
    single_species_fourier_transform_grapher(
        min_time,
        max_time,
        timestep,
        applied_field,
...
def all_species_correlator_to_fourier(
    min_time, max_time, timestep=10e-9, applied_B_field_list=[0.0, 0.5, 1.0]
):
    """
    ...
    for applied_B_field in applied_B_field_list:
        ...
        linear_spaced_correlator_simulation(
            min_time, max_time, timestep, applied_B_field
        )
        print(f"Finished correlator computation for B = {applied_B_field}T.")

        # plot and save the graohs for all species
        single_species_time_correlator_grapher(
            min_time, max_time, n_times, applied_B_field, "Ga69"
        )
        single_species_time_correlator_grapher(
            min_time, max_time, n_times, applied_B_field, "Ga71"
        )
        single_species_time_correlator_grapher(
            min_time, max_time, n_times, applied_B_field, "As75"
        )
        single_species_time_correlator_grapher(
            min_time, max_time, n_times, applied_B_field, "In115"
        )
...
def all_species_logtime_correlator_grapher(
    min_exp, max_exp, n_times=1000, applied_B_field_list=[0.0, 0.5, 1.0]
):
    """
    Runs an entire simulation, for all species along all axes.
    ...
    for applied_B_field in applied_B_field_list:
        ...
        single_species_time_correlator_grapher(
            exp_min_time, exp_max_time, n_times, applied_B_field, "Ga69", logtime=True
        )
        single_species_time_correlator_grapher(
            exp_min_time, exp_max_time, n_times, applied_B_field, "Ga71", logtime=True
        )
        single_species_time_correlator_grapher(
            exp_min_time, exp_max_time, n_times, applied_B_field, "As75", logtime=True
        )
        single_species_time_correlator_grapher(
            exp_min_time, exp_max_time, n_times, applied_B_field, "In115", logtime=True
        )
        print(f"Finished Correlator Graphs for B = {applied_B_field}T.")
...
if __name__ == "__main__":
    ...
    for nuclear_species in nuclear_species_list:
        single_species_correlator_to_fourier(
            start_time,
            end_time,
            time_increment,
            applied_magnetic_field,
            nuclear_species,
            step_size=site_interval,
            allocated_at_once=tasks_allocated,
        )
        single_species_linear_time_correlator_grapher(
            start_time,
            end_time,
