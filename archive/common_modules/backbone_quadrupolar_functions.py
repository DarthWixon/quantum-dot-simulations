...
def load_mirrored_data(region_bounds, step_size=1, faked_strain_type="left_right"):
    ...
    full_archive = np.load(
        f"{data_path}{faked_strain_type}_mirrored_strain_data_in_region_{region_bounds}.npz"
    )
...
def calculate_EFG(
    nuclear_species, xx_array, xz_array, zz_array, use_sundfors_GET_vals=False
):
    """
    ...
    for i in range(n):
        for j in range(m):
            ...
            V = np.array(
                [
                    [S12 * (zz_array[i, j] - xx_array[i, j]), 0, S44 * xz_array[i, j]],
                    [
                        0,
                        (S12 + S11) * xx_array[i, j] + S12 * zz_array[i, j],
                        S44 * xz_array[i, j],
                    ],
                    [
                        S44 * xz_array[i, j],
                        S44 * xz_array[i, j],
                        2 * S12 * xx_array[i, j] + S11 * zz_array[i, j],
                    ],
                ]
            )
...
def calculate_and_save_EFG(
    ...
    if real_strain_data:
        ...
        if not use_sundfors_GET_vals:
            archive_name = (
                data_path
                + nuclear_species
                + "_calculation_results_for_region"
                + str(region_bounds)
                + f"_with_step_size_{step_size}.npz"
            )
        else:
            archive_name = (
                data_path
                ...
                + "_calculation_results_for_region"
                + str(region_bounds)
                + f"_with_step_size_{step_size}_with_old_params.npz"
            )
    else:
        xx_array, xz_array, zz_array = load_mirrored_data(
            region_bounds, step_size, faked_strain_type
        )
        ...
        if not use_sundfors_GET_vals:
            archive_name = (
                ...
                + "_calculation_results_for_region"
                + str(region_bounds)
                + f"_with_step_size_{step_size}_using_{faked_strain_type}_flipped_data.npz"
            )
        else:
            archive_name = (
                data_path
                ...
                + "_calculation_results_for_region"
                + str(region_bounds)
                + f"_with_step_size_{step_size}_using_{faked_strain_type}_flipped_data_with_old_params.npz"
            )
    ...
    eta, V_XX, V_YY, V_ZZ, euler_angles = calculate_EFG(
        nuclear_species, xx_array, xz_array, zz_array, use_sundfors_GET_vals
    )
...
def load_calculated_EFG_arrays(
    nuclear_species,
    region_bounds,
    step_size=1,
    use_sundfors_GET_vals=False,
...
def structural_hamiltonian_creator_Faraday(
    ...
    nuclear_hamiltonian = Zeeman_term * I_z + quadrupolar_term * (
        2 * I_z_quad**2
        + (biaxiality - 1) * I_x_quad**2
        - (biaxiality + 1) * I_y_quad**2
    )

    return nuclear_hamiltonian


def structural_hamiltonian_creator_Voigt(
    Zeeman_term, quadrupolar_term, biaxiality, particle_spin, alpha, beta, gamma
):
    """
...
def spin_correlator_matrix_exponential(t, nuclear_hamiltonian, spin_axis):
    ...
    if spin_axis == "x":
        ...
        print(
            f"bqf.spin_correlator_matrix_exponential recieved an invalid spin axis. Options are x, y, z."
        )
        return None
    ...
    spin_correlator = numpy.real_if_close(
        (U_plus * spin_operator * U_minus * spin_operator * rho).tr()
    )

    return spin_correlator


def site_correlator_calculator_not_parallel(
    t,
    ...
    Hamiltonian = structural_hamiltonian_creator_Faraday(
        zeeman_frequency, quadrupole_frequency, biaxiality, spin, alpha, beta, gamma
    )
    correlator_at_site = spin_correlator_matrix_exponential(
        t, Hamiltonian, correlator_axis
    )

    return correlator_at_site


def parallel_site_correlator_calculator(
    elapsed_time,
    zeeman_frequency_per_tesla,
    quadrupole_coupling_constant,
    spin,
    ...
    correlator_at_site = spin_correlator_matrix_exponential(
        elapsed_time, Hamiltonian, correlator_axis
    )
