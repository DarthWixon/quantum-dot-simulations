...
def find_best_locations(n_locs, region_bounds_label, search_range, testing=False):
    ...
    for loc in loc_list:
        ...
        relative_area_max_coords = np.unravel_index(
            np.argmax(search_area, axis=None), search_area.shape
        )

        area_max_coords = tuple(
            np.add(np.add(relative_area_max_coords, np.array(loc)), -1 * search_range)
        )
...
def checking_it_works():
    ...
    for i in range(10):
        random_locs, high_points = find_best_locations(
            n_locs, region_bounds_label, search_range, testing=True
        )
    ...
    print(
        f"Random point X coord higher {(x_higher/x_tot)*100}% of the time. It's lower {(x_lower/x_tot)*100}% of the time. They are equal {(x_equal/x_tot)*100}% of the time. {x_tot} points tested."
    )
    print(
        f"Random point Y coord higher {(y_higher/y_tot)*100}% of the time. It's lower {(y_lower/y_tot)*100}% of the time. They are equal {(x_equal/y_tot)*100}% of the time. {y_tot} points tested."
    )
